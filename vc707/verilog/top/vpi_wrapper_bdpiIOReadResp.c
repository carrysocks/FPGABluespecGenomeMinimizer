 ...
 *     #include "vpi_wrapper_bdpiIOReadResp.h"
 *     void (*vlog_startup_routines[])() = { bdpiIOReadResp_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_bdpiIOReadResp call=bdpiIOReadResp_calltf size=1 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_bdpiIOReadResp vpiSysFuncSized 1 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
PLI_INT32 bdpiIOReadResp_calltf(PLI_BYTE8 *user_data)
{
  ...
  vpi_result = bdpiIOReadResp(arg_1);
...
/* sft: $imported_bdpiIOReadResp vpiSysFuncSized 1 unsigned */

/* tab: $imported_bdpiIOReadResp call=bdpiIOReadResp_calltf size=1 acc=rw:%TASK */

PLI_INT32 bdpiIOReadResp_sizetf(PLI_BYTE8 *user_data)
{
  return 1u;
}
...
void bdpiIOReadResp_vpi_register()
{
  ...
  tf_data.tfname = "$imported_bdpiIOReadResp";
  tf_data.calltf = bdpiIOReadResp_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = bdpiIOReadResp_sizetf;
  tf_data.user_data = "$imported_bdpiIOReadResp";
