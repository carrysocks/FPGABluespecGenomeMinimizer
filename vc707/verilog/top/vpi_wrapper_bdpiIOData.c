 ...
 *     #include "vpi_wrapper_bdpiIOData.h"
 *     void (*vlog_startup_routines[])() = { bdpiIOData_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_bdpiIOData call=bdpiIOData_calltf size=64 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_bdpiIOData vpiSysFuncSized 64 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
PLI_INT32 bdpiIOData_calltf(PLI_BYTE8 *user_data)
{
  vpiHandle hCall;
...
/* sft: $imported_bdpiIOData vpiSysFuncSized 64 unsigned */

/* tab: $imported_bdpiIOData call=bdpiIOData_calltf size=64 acc=rw:%TASK */

PLI_INT32 bdpiIOData_sizetf(PLI_BYTE8 *user_data)
{
  return 64u;
}

/* VPI wrapper registration function */
void bdpiIOData_vpi_register()
{
  ...
  tf_data.tfname = "$imported_bdpiIOData";
  tf_data.calltf = bdpiIOData_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = bdpiIOData_sizetf;
  tf_data.user_data = "$imported_bdpiIOData";
