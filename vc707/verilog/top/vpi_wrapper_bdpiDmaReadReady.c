 ...
 *     #include "vpi_wrapper_bdpiDmaReadReady.h"
 *     void (*vlog_startup_routines[])() = { bdpiDmaReadReady_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_bdpiDmaReadReady call=bdpiDmaReadReady_calltf size=1 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_bdpiDmaReadReady vpiSysFuncSized 1 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
/* sft: $imported_bdpiDmaReadReady vpiSysFuncSized 1 unsigned */

/* tab: $imported_bdpiDmaReadReady call=bdpiDmaReadReady_calltf size=1 acc=rw:%TASK */

PLI_INT32 bdpiDmaReadReady_sizetf(PLI_BYTE8 *user_data)
{
  return 1u;
}
...
void bdpiDmaReadReady_vpi_register()
{
  ...
  tf_data.tfname = "$imported_bdpiDmaReadReady";
  tf_data.calltf = bdpiDmaReadReady_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = bdpiDmaReadReady_sizetf;
  tf_data.user_data = "$imported_bdpiDmaReadReady";
