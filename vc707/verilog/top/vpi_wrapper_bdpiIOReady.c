 ...
 *     #include "vpi_wrapper_bdpiIOReady.h"
 *     void (*vlog_startup_routines[])() = { bdpiIOReady_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_bdpiIOReady call=bdpiIOReady_calltf size=1 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_bdpiIOReady vpiSysFuncSized 1 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
/* sft: $imported_bdpiIOReady vpiSysFuncSized 1 unsigned */

/* tab: $imported_bdpiIOReady call=bdpiIOReady_calltf size=1 acc=rw:%TASK */

PLI_INT32 bdpiIOReady_sizetf(PLI_BYTE8 *user_data)
{
  return 1u;
}
...
void bdpiIOReady_vpi_register()
{
  ...
  tf_data.tfname = "$imported_bdpiIOReady";
  tf_data.calltf = bdpiIOReady_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = bdpiIOReady_sizetf;
  tf_data.user_data = "$imported_bdpiIOReady";
