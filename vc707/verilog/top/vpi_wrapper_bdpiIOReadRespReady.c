 ...
 *     #include "vpi_wrapper_bdpiIOReadRespReady.h"
 *     void (*vlog_startup_routines[])() = { bdpiIOReadRespReady_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_bdpiIOReadRespReady call=bdpiIOReadRespReady_calltf size=1 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_bdpiIOReadRespReady vpiSysFuncSized 1 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
PLI_INT32 bdpiIOReadRespReady_calltf(PLI_BYTE8 *user_data)
{
  vpiHandle hCall;
  char vpi_result;
  ...
  vpi_result = bdpiIOReadRespReady();
...
/* sft: $imported_bdpiIOReadRespReady vpiSysFuncSized 1 unsigned */

/* tab: $imported_bdpiIOReadRespReady call=bdpiIOReadRespReady_calltf size=1 acc=rw:%TASK */

PLI_INT32 bdpiIOReadRespReady_sizetf(PLI_BYTE8 *user_data)
{
  return 1u;
}

/* VPI wrapper registration function */
void bdpiIOReadRespReady_vpi_register()
{
  ...
  tf_data.tfname = "$imported_bdpiIOReadRespReady";
  tf_data.calltf = bdpiIOReadRespReady_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = bdpiIOReadRespReady_sizetf;
  tf_data.user_data = "$imported_bdpiIOReadRespReady";
